# Script test booking sau khi fix lỗi type casting
Write-Host "=== TEST BOOKING AFTER FIX TYPE CASTING ===" -ForegroundColor Green
...
$bookingBody = @{
    ...
    notes = "Test booking sau khi fix type casting"
} | ConvertTo-Json
...
Write-Host "✓ Lỗi type casting đã được fix!" -ForegroundColor Green
Write-Host "✓ Booking flow hoạt động bình thường!" -ForegroundColor Green
Write-Host "✓ Tất cả API đã hoạt động đúng!" -ForegroundColor Green
